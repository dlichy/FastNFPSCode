#adapted from https://github.com/guanyingc/SDPS-Net
...
class Classifier(nn.Module):
    def __init__(self, batchNorm, c_in):
        ...
        self.x_est = nn.Sequential(
                    model_utils.conv(batchNorm, 256, 64,  k=1, stride=1, pad=0),
                    model_utils.outputConv(64, 1, k=1, stride=1, pad=0))

        self.y_est = nn.Sequential(
                    model_utils.conv(batchNorm, 256, 64,  k=1, stride=1, pad=0),
                    model_utils.outputConv(64, 1, k=1, stride=1, pad=0))

        self.z_est = nn.Sequential(
                    model_utils.conv(batchNorm, 256, 64,  k=1, stride=1, pad=0),
                    model_utils.outputConv(64, 1, k=1, stride=1, pad=0))
    ...
    def forward(self, inputs):
        ...
        out = self.conv3(out)
        out = self.conv4(out)
        outputs = torch.cat([self.x_est(out).squeeze(3).squeeze(2),self.y_est(out).squeeze(3).squeeze(2), self.z_est(out).squeeze(3).squeeze(2)],dim=1)
        return outputs
